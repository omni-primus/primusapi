...
var text2;
var text;
var requestURL = 'https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/'+name+'?api_key='+api;
...
router.get('/', (req, res, next) => {
  res.status(200).json({
      message: 'You forgot to enter a Summoner Name'
  });
});
...
router.get('/:SummonerName', (req, res, next) => {
  ...
  getELo();
  setTimeout(function(){
      if (SName === 'omni_primus'){
          res.status(200).json({
              message: text
          });
      } else {
          res.status(200).json({
              message: 'this streamer is not supported'
          })
      }
  }, 1200);
});
